...
import os
import sys
output_dir = "./outputs_optimized/dc12training/sub_inter/"
...
class VanillaModel(nn.Module):
    def __init__(self, p, dropconnect_p=0.8):
        ...
        self.coordinates1 = torch.rand(250, 2)
        self.coordinates2 = torch.rand(250, 2)
        ...
        for i in self.sequence:
            for j in range(250):
                if j not in self.activated_before[i] and i != j:
                    dx = self.coordinates1[i][0] - self.coordinates2[j][0]
                    dy = self.coordinates1[i][1] - self.coordinates2[j][1]
    ...
    def forward(self, x):
        ...
        x = x.squeeze(0)
        copy1_x = x.detach()
        eph = torch.matmul(copy1_x.to(device), self.ephaptic_matrix) 
        ...
        if self.training:
            train_synaptic[self.epoch - 1] = copy2_x.cpu().tolist()
        else:
            test_synaptic[self.epoch - 1] = copy2_x.cpu().tolist()
...
for epoch in range(num_epochs):
    ...
    plt.tight_layout()
    plt.savefig(f'synaptic_total_ratio_histogram_epoch_{epoch + 1}.png')
    plt.close()
